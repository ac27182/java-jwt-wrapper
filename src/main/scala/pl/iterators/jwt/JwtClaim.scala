...
final case class JwtClaim[+C](
  ...
  def toJwt[C1 >: C](
      algorithm: Algorithm
  )(implicit claims: Claims[C1]): String =
    claims.encoder
      .encode(
        List(iss, sub, exp, nbf, iat, jti, aud)
          .foldLeft(JWT.create())((builder, publicClaim) =>
            publicClaim.toJwt(builder)
          )
...
object JwtClaim {
  ...
  def fromJwt[C](
      ...
      jti: JwtId = JwtId(),
      aud: Audience = Audience()
  )(jwt: String)(implicit claims: Claims[C]): Try[JwtClaim[C]] = {
    val publicClaims =
      List(iss, sub, ExpiresAt(), NotBefore(), IssuedAt(), jti, aud)
    Try {
      publicClaims
      ...
      publicClaims.foldLeft(
        JwtClaim(claims.of(claims.decoder.decode(decoded.getClaims)))
  ...
  sealed abstract class PublicClaim[T] {
    ...
    final def toJwt(builder: Builder): Builder =
      value.fold(builder)(v => withSelf(builder, v))
  ...
  object PublicClaims {
    final case class Issuer(value: Option[String] = None)
        extends PublicClaim[String] {
      override type Self = Issuer
      override def set(value: String) = copy(Some(value))
      override def get(decodedJWT: DecodedJWT) =
        Issuer(Option(decodedJWT.getIssuer))
      override def setSelf[A](jwtClaim: JwtClaim[A]) = jwtClaim.copy(iss = this)
      override protected def withSelf(builder: Builder, t: String) =
        builder.withIssuer(t)
      override protected def requireSelf(
          verification: Verification,
          t: String
      ) =
        verification.withIssuer(t)
    }
    final case class Subject(value: Option[String] = None)
        extends PublicClaim[String] {
      override type Self = Subject
      override def set(value: String) = copy(Some(value))
      override def get(decodedJWT: DecodedJWT) =
        Subject(Option(decodedJWT.getSubject))
      override def setSelf[A](jwtClaim: JwtClaim[A]) = jwtClaim.copy(sub = this)
      override protected def withSelf(builder: Builder, t: String) =
        builder.withSubject(t)
      override protected def requireSelf(
          verification: Verification,
          t: String
      ) =
    ...
    final case class JwtId(value: Option[String] = None)
        extends PublicClaim[String] {
    ...
    sealed abstract class NumericDatePublicClaim extends PublicClaim[Instant] {
      ...
      protected final def instantValue(maybeDate: Option[Date]) =
        maybeDate.map(_.toInstant)
      protected final def truncated(instant: Instant) =
      ...
      override final protected def requireSelf(
          verification: Verification,
          t: Instant
      ) =
        verification
    }
    final case class ExpiresAt(value: Option[Instant] = None)
        extends NumericDatePublicClaim {
    ...
    final case class NotBefore(value: Option[Instant] = None)
        extends NumericDatePublicClaim {
    ...
    final case class IssuedAt(value: Option[Instant] = None)
        extends NumericDatePublicClaim {
    ...
    final case class Audience(value: Option[Set[String]] = None)
        extends PublicClaim[Set[String]] {
