...
class JwtClaimSpecs extends FlatSpec with Matchers {
  ...
  object GenericEncodeDecodeBehaviors {
    ...
    def assertDecoded[C: Claims](
      ...
      val decodedOrError =
        JwtClaim.fromJwt(algorithm, iss = iss, sub = sub, jti = jti, aud = aud)(
          token
        )
    ...
    final class AssertFailedPartiallyApplied[Ex <: Throwable] {
      ...
      def apply[C: Claims](
          ...
          jti: JwtId = JwtId(),
          aud: Audience = Audience(),
          decodingAlgorithm: Algorithm = algorithm
      )(implicit classTag: ClassTag[Ex]): Assertion = {
        val token = beforeEncoding.toJwt(algorithm)
        ...
        decodedOrError match {
          case Success(_) =>
            fail(
              s"Claim $beforeEncoding was decoded successfully even if it should not"
            )
  ...
  case class ClaimsWithTraversable(
      ss: List[String],
      ls: List[Long],
      is: List[Int]
  )
  ...
  "JwtClaim with Claims (simple values)" should behave like JwtToken(
    SimpleClaims(
      ...
      "whatnot",
      false,
      Date.from(Instant.now().truncatedTo(SECONDS))
    )
  )
  ...
  "JwtClaim with Claims (options))" should behave like JwtToken(
    ClaimsWithOptions(Some(100), None)
  )

  "JwtClaim with Claims (mapped))" should behave like JwtToken(
    MappedClaims(UUID.randomUUID())
  )
  ...
  implicit val byHandClaims =
    Claims.of[Record.`"name" -> String, "id" -> Int`.T]
  ...
  it should "fail to decode if wrong algorithm is used" in assertFailed[
  ...
  it should "fail to decode if required claim is not set" in assertFailed[
  ...
  it should "fail to decode if required claim is missing" in assertFailed[
    InvalidClaimException
  ](
    JwtClaim(("name" ->> "Marcin") :: ("id" ->> 3) :: HNil)
      .about("Marcin"),
  ...
  "JwtClaim with Claims (sealed trait hierarchy #1))" should behave like JwtToken[
    Hierarchy
  ](Class1(10))
  "JwtClaim with Claims (sealed trait hierarchy #2))" should behave like JwtToken[
    Hierarchy
  ](Class2("10"))
